...
verify_root="$(mktemp -d "${RUNNER_TEMP:-/tmp}/camellia-linux-verification.XXXXXX")"
trap 'rm -rf -- "$verify_root"' EXIT
mkdir -m 700 "$verify_root/keyring"
GNUPGHOME="$verify_root/keyring" gpg --batch --quiet --import "$LINUX_GPG_PUBLIC_KEY" >/dev/null 2>&1
if ! GNUPGHOME="$verify_root/keyring" gpg --batch --with-colons --with-fingerprint \
  --list-keys "$fingerprint" 2>/dev/null |
  awk -F: -v expected="$fingerprint" '$1 == "fpr" && toupper($10) == expected { found = 1 } END { exit !found }'; then

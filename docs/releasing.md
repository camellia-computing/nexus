...
- Complete Linux OpenPGP group: sign the AppImage, Debian package and portable archive with ASCII-armored detached signatures and publish only those three signatures plus the matching public key under the fixed public name `RELEASE-SIGNING-KEY.asc`.
- Any partial or contradictory group: fail before packaging.

See [Windows code signing](windows-code-signing.md), [macOS code signing](macos-code-signing.md) and [Linux artifact signing](linux-artifact-signing.md). Native and optional detached signing are independent from supply-chain signing. Before protected-environment approval, the workflow freezes the nine packages, native metadata/report, SPDX SBOM, GitHub provenance and SBOM attestations, and the organization-standard `release-evidence.json`; it then uploads that exact set as one immutable workflow artifact. After approval, every frozen file receives a keyless Cosign bundle. Schema 3 internal release metadata records the exact commit, per-platform native-signing state, verifier-derived trust classification, reviewed identity, artifact-signing scheme/trust and delivery mode. The deterministic human-readable report is regenerated and byte-compared from that metadata before publication. Current non-secret identities and rotation state remain in the organization governance repository's signing registry; secret key material never enters it.

The internal evidence set is nine platform packages, `RELEASE-METADATA.json`, `NATIVE-SIGNING.md`, `SBOM.spdx.json`, the two attestation bundles, `release-evidence.json` and `SHA256SUMS`, plus four Linux OpenPGP files only when metadata enables that scheme. Every internal raw file keeps its keyless Cosign bundle, and the complete set remains in the retained Actions Artifact. Package names follow `camellia-nexus-<version>-<platform>-<architecture>.<format>`, except that the Windows application/broker archive uses the explicit `-portable.zip` suffix to distinguish it from Unix archives. Before copying package artifacts into the evidence directory, the workflow compares the complete input basename multiset with the metadata-derived expected list, so missing, unexpected and duplicate packages or signatures fail before an overwrite can hide them.

The public GitHub Release whitelist is deliberately narrower: the exact nine installable/portable packages and `SHA256SUMS`. `SHA256SUMS` covers exactly those nine packages. The publisher constructs this set from exact expected basenames rather than directory or extension globs. If and only if metadata enables Linux OpenPGP signing, the public set additionally contains the exact three `<linux-final-artifact>.asc` detached signatures and `RELEASE-SIGNING-KEY.asc`; no `*.asc` glob is used. `SBOM.spdx.json`, `*.sigstore.json`, `*.intoto.jsonl`, `release-evidence.json`, `RELEASE-METADATA.json`, `NATIVE-SIGNING.md` and other CI evidence continue to be produced and validated but are not ordinary Release Assets. During an authorized draft retry the publisher may replace only an expected public asset whose bytes conflict with the frozen build; any unexpected public asset fails closed. Once public, no asset is replaced or rebuilt.

Before publication the publisher validates the complete internal evidence set, every evidence digest and every Cosign bundle. It then stages only the public whitelist in an isolated directory, prints that selection, and checks its package-only checksum manifest. After publication it downloads the exact public set, verifies all checksums and both GitHub attestations for all nine packages, and performs full-fingerprint OpenPGP verification when configured. `latest` is reconciled only after completion and always points to the highest completed immutable stable SemVer, so recovering an older release cannot roll it back. Immutable Releases are required; [GitHub permits release-note updates](https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository#editing-a-release) while protecting the associated tag and assets.
...
- An incomplete publication must retain `release:pending`. Once the immutable `release-complete:<SHA>` marker has been validated, recovery accepts the authorizing Release PR immediately before or after label cleanup; exact PR identity, reviewed head, approval, merge topology, tag and completion marker remain mandatory.

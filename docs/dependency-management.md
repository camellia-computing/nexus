...
| `brace-expansion` 5.0.9 root override and compatibility patch | `GHSA-rgw5-rvv9-x895` requires the current 5.0.9 security release, which also retains the limits introduced for `GHSA-mh99-v99m-4gvg`. WebdriverIO 9.30.0 still reaches `minimatch` 3.1.5, 5.1.9 and 9.0.9, whose v1/v2 dependency and callable/default import contracts are incompatible with the current named v5 API. Downgrading violates the current-version and security policies. The exact pnpm patch restores only the legacy module surface while retaining the official 5.0.9 implementation and limits. This graph is test-only and is not part of the Tauri `ui/dist` release payload; frozen installation, dependency audit, the driver contract and native Windows E2E remain mandatory. | Desktop maintainers | 2026-08-18 | Remove the patch and override as one transaction when the supported current WebdriverIO graph natively consumes the current `brace-expansion` API, or the current `brace-expansion` release officially supplies the required compatibility surface. Regenerate the lockfile, confirm no older incompatible `minimatch` remains, and run audit plus browser and native Windows suites. |
